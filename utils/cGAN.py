...
class cGAN :
        ...
        for epoch in range(self.start_epochs,self.epochs) :
            ...
            with tqdm(total=len(trainloader),\
                ...
                for i,data in enumerate(trainloader) :
                    ...
                    output_vae_orig, loss_GT, loss_hvs = self.fitG(inputG,imgsH)
                    ...
                    # tqdm update
                    t.set_postfix_str('B G loss : %.4f'%(loss_GT)+\
                                    ', B D loss : %.4f'%((loss_D)/2)+\
                                    ', B G HVS loss : %.4f'%(loss_hvs))
                    t.update()
    ...
    def fitG(self,inputG,imgsH) :

        # generated image for cGAN
        output_vae_orig = self.netG(inputG)
        ...
        loss_GT = self.GT_loss(imgsH,output_vae_orig)
